    ...
    <div class="pt-6 pb-16 bg-light text-dark">
      <div class="container flex flex-wrap mx-auto xl:max-w-screen-xl">
        ...
        <div class="w-full mt-10 md:w-1/3">
          ...
          >
            <!-- LOGO -->
            <div class="flex items-center justify-center mr-0 md:mr-10">
              <img :src="getLogoSrc" :alt="project.title" class="w-48 h-auto" />
            </div>
            <!-- PROJECT -->
            <div class="mt-8 mr-0 text-xl font-bold text-indigo-600 md:mr-10">
            ...
            <div v-if="$t(`projects.${project.slug}.functionnalities`).length">
              <div class="mt-6 mr-0 text-xl font-bold text-indigo-600 md:mr-10">
                {{ $t("portfolio.projects.functionnalities") }}
              </div>
              <div class="mt-3 mr-0 md:mr-10">
                <ul>
                  <li
                    v-for="(functionnality, index) in $t(
                      `projects.${project.slug}.functionnalities`
                    )"
                    :key="index"
                  >
                    - {{ functionnality }}
            ...
            <div v-if="$t(`projects.${project.slug}.specifications`).length">
              ...
              <div class="mt-3 mr-0 md:mr-10">
                <ul>
                  <li
                    v-for="(specification, index) in $t(
                      `projects.${project.slug}.specifications`
                    )"
                    :key="index"
                  >
                    - {{ specification }}
            ...
            <!-- LINKS -->
            <div
              class="flex items-center justify-center w-full mt-8 md:justify-start"
            >
              <nuxt-link to="/portfolio">
                <button
                  class="px-4 py-2 mr-2 text-base font-medium tracking-wide text-gray-700 uppercase transition-colors duration-100 transform bg-gray-100 rounded-md md:text-sm lg:text-base md:mr-2 hover:bg-gray-200 focus:outline-none"
                >
              ...
              >
                <span v-if="project.types.includes('WEB')">{{
                  $t("portfolio.projects.website")
                }}</span>
                <span v-if="project.types.includes('APP')">{{
                  $t("portfolio.projects.app")
                }}</span>
                <span v-if="project.types.includes('ANIMATION')">{{
                  $t("portfolio.projects.animation")
                }}</span>
              </button>
            </div>
          </div>
        ...
        <div class="w-full mt-10 md:w-2/3">
          <div
            v-if="
              $t(`projects.${project.slug}.video`) &&
              !$t(`projects.${project.slug}.videoVertical`)
            "
...
export default {
  ...
  mounted() {
    window.scrollTo(0, 0);
    this.selectedIdx = this.filteredProjects.findIndex(
  ...
  methods: {
    ...
    nextProject() {
      this.selectedIdx >= this.filteredProjects.length - 1
        ? (this.selectedIdx = 0)
        : this.selectedIdx++;
      this.$router.push(
        `/portfolio/${this.filteredProjects[this.selectedIdx].slug}`
      );
    },
    previousProject() {
      this.selectedIdx <= 0
